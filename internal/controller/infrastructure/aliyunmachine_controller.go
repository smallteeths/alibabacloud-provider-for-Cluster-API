...
func (r *AliyunMachineReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	pcName, err := r.reconcileProviderConfig(ctx, log, am)
	if err != nil {
		return ctrl.Result{}, err
	}
	...
	want := r.buildDesiredInstance(am, pcName, userDataB64, instName)
	...
	isCP, ownerMachine, err := r.isControlPlaneMachine(ctx, am)
	if err != nil {
		log.Info("failed to check if control-plane", "err", err)
	} else if isCP {
		cl, err := util.GetClusterFromMetadata(ctx, r.Client, ownerMachine.ObjectMeta)
		if err != nil || cl == nil {
			log.Info("finding cluster", "err", err)
			return ctrl.Result{RequeueAfter: 15 * time.Second}, nil
		}

		if cl.Spec.InfrastructureRef == nil ||
			cl.Spec.InfrastructureRef.Kind != "AliyunCluster" ||
			cl.Spec.InfrastructureRef.APIVersion != "infrastructure.cluster.x-k8s.io/v1beta2" {
			log.Info("Can not find cluster", "type", cl.Spec.InfrastructureRef.Kind)
		} else {
			ac := &infrav1beta2.AliyunCluster{}
			if err := r.Client.Get(ctx, types.NamespacedName{
				Namespace: cl.Namespace,
				Name:      cl.Spec.InfrastructureRef.Name,
			}, ac); err == nil {
				instID := *inst.Status.AtProvider.ID
				if len(ac.Status.ServerGroupIDs) == 0 {
					log.Info("waiting for ServerGroup to create")
					return ctrl.Result{RequeueAfter: 15 * time.Second}, nil
				}
				res, err := r.attachInstanceToServerGroups(ctx, ac, am.Spec.RegionID, instID, log)
				if err != nil {
					return res, err
				}
...
func (r *AliyunMachineReconciler) reconcileProviderConfig(
	...
	if region == "" {
		return "", fmt.Errorf("region not set: add annotation %q or configure Reconciler.CredentialSecret.Region", RegionAnnotation)
	}
	...
	if err := ensureUpjetProviderConfigNoStore(
		ctx, r.Client, "alibabacloud.alibabacloud.com", "v1beta1",
		region, region, secretKey.Namespace, secretKey.Name, r.CredentialSecret.Key,
	); err != nil {
		return "", err
	}
	return region, nil
...
func (r *AliyunMachineReconciler) isControlPlaneMachine(
	ctx context.Context, am *infrav1beta2.AliyunMachine,
) (bool, *clusterv1.Machine, error) {
...
func (r *AliyunMachineReconciler) attachInstanceToServerGroups(
	ctx context.Context, ac *infrav1beta2.AliyunCluster, region, instanceID string, log logr.Logger,
) (ctrl.Result, error) {
	sdkClient, err := clients.CreateNlbSDKClient(region)
	if err != nil {
		return ctrl.Result{}, err
	}
	serverType := "Ecs"
	lsrList := &nlbv1alpha1.ListenerList{}
	if err := r.Client.List(ctx, lsrList,
	...
	for _, lsr := range lsrList.Items {
		servers0 := &nlb.AddServersToServerGroupRequestServers{
			Port:       f64PtrToI32PtrUnsafe(lsr.Status.AtProvider.ListenerPort),
			ServerId:   tea.String(instanceID),
			ServerType: tea.String(serverType),
		}
		addServersToServerGroupRequest := &nlb.AddServersToServerGroupRequest{
			RegionId:      tea.String(region),
			Servers:       []*nlb.AddServersToServerGroupRequestServers{servers0},
			ServerGroupId: lsr.Status.AtProvider.ServerGroupID,
		}
		runtime := &dara.RuntimeOptions{}
		_, err := sdkClient.AddServersToServerGroupWithOptions(addServersToServerGroupRequest, runtime)
		if err != nil {
			log.Error(err, "Add Nlb Servers To ServerGroup error")
			continue
		}
	}
